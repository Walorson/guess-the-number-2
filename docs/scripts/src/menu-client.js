...
export function connectToServer() {
  ...
  socket.on("removePlayerFromWaitingRoom", (nickname2) => {
    waitingRoom.querySelectorAll("button").forEach((button) => {
      if (button.textContent == nickname2) {
        button.textContent = ".";
...
function onlinePlayers() {
  ...
  socket.on("getOnlinePlayers", (onlinePlayers2) => {
    onlinePlayersDiv.textContent = "Online: " + onlinePlayers2;
  });
}
